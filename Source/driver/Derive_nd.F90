...
! inside OpenMP parallel regions.
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             vel(i,j,k,1) = q(i,j,k,2) + eos_state % cs
          enddo
       enddo
    enddo
  ...
  subroutine ca_derkineng(kineng,k_lo,k_hi,nk, &
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             kineng(i,j,k,1) = HALF * ( q(i,j,k,2)**2 + &
                  q(i,j,k,3)**2 + &
                  q(i,j,k,4)**2 )
          end do
       end do
  ...
  subroutine ca_derprim_w(u,u_lo,u_hi,ncomp_u, &
                        dat,d_lo,d_hi,nc,&
                        lo,hi,domlo, &
                        domhi,dx,xlo,time,dt,bc,level,grid_no) &
                        bind(C, name="ca_derprim_w")
    ! calculate the Lorentz factor

    use bl_constants_module
    use meth_params_module, only: NQ, NQAUX, NVAR, QW
    use metric_module, only: calculate_scalar_W, calculate_gamma_up
    ...
    do k = lo(3), hi(3)
      do j = lo(2), hi(2)
          do i = lo(1), hi(1)
              u(i,j,k,1) = q(i,j,k,QW)
          end do
      end do
    end do

end subroutine ca_derprim_w
