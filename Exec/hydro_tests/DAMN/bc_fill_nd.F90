  ...
  subroutine ca_hypfill(adv,adv_lo,adv_hi, &
    ...
    do n = 1, NVAR
       ...
       if ( bc(3,1,n).eq.EXT_DIR .and. adv_lo(3).lt.domlo(3)) then
          ...
          ! we are periodic in z -- we should never get here
          call bl_error("ERROR: invalid BC in Prob_nd.f90")
                 ...
                 do i= domlo(1)-1, adv_lo(1)-1
                    ...
                    if (n .eq. URHO) then
                       ...
                       if (interp_BC) then
                          ...
                          ! initial guesses
                          dens_zone = adv(i+1,j,URHO)

                          ! temperature and species held constant in BCs
                          temp_zone = adv(i+1,j,UTEMP)
                          X_zone(:) = adv(i+1,j,UFS:UFS-1+nspec)/adv(i+1,j,URHO)

                          ! get pressure in zone above
                          eos_state%rho = adv(i+1,j,URHO)
                          eos_state%T = adv(i+1,j,UTEMP)
                          eos_state%xn(:) = adv(i+1,j,UFS:UFS-1+nspec)/adv(i+1,j,URHO)
                          ...
                          do iter = 1, MAX_ITER
                             ...
                             p_want = pres_above - &
                                  delta(1)*0.5e0_rt*(dens_zone + adv(i+1,j,URHO))*g
                       ...
                       if (zero_vels) then
                          ...
                          adv(i,j,UMX) = dens_zone*(adv(domlo(1),j,UMX)/adv(domlo(1),j,URHO))
                          adv(i,j,UMY) = dens_zone*(adv(domlo(1),j,UMY)/adv(domlo(1),j,URHO))
                          adv(i,j,UMZ) = dens_zone*(adv(domlo(1),j,UMZ)/adv(domlo(1),j,URHO))
                       endif
              ...
              do j=adv_lo(2),adv_hi(2)
                 ...
                 do i=domhi(1)+1,adv_hi(2)
                    ...
                    if (n .eq. URHO) then
                       ...
                       adv(i,j,UMY) = max(0.e0_rt,adv(domhi(1),j,UMY))
       ...
       do k=adv_lo(3),adv_hi(3)
           do j=adv_lo(2),adv_hi(2)
              do i=adv_lo(1),domlo(1)-1
                 x = xlo(1) + delta(1)*(dble(i-adv_lo(1)) + 0.5e0_rt)
                 adv(i,j) = interpolate(x,npts_model,model_r,model_state(:,idens_model))
              end do
           end do
        end do
    end if
